...
<button onClick={refetch} className="mt-4 bg-blue-600 text-white px-6 py-2 rounded-md">
Refresh Prices
</button>
...
filteredCrypto.map(([coin, details]) => (
<div key={coin} className="flex justify-between bg-white p-4 rounded-md my-2">
<span className="font-medium text-black">{coin.toUpperCase()}</span>
<span className="font-medium text-black">${details.usd.toFixed(2)}</span>
</div>
))
) : (
<p className="text-black">No matching results.</p>
)}
</div>
</div>
);
